...
export class ReAuthEngine {
  private container: AwilixContainer<ReAuthCradleExtension>;
  ...
  getContainer(): AwilixContainer<ReAuthCradleExtension> {
  ...
  // Broad signature to match EngineApi for step context
  async executeStep(
    pluginName: string,
    stepName: string,
    input: unknown,
  ...
  async executeStep(
    ...
    const ctx = {
      ...
      engine: {
        ...
        getUnifiedProfile: (subjectId: string) =>
          this.getUnifiedProfile(subjectId),
      },
  ...
  registerSessionHook(
    ...
    fn: (
      data: AuthInput | AuthOutput,
      container: AwilixContainer<ReAuthCradleExtension>,
      error?: unknown,
    ) => Promise<AuthInput | AuthOutput | void> | AuthInput | AuthOutput | void,
    universal?: boolean,
  ): this {
    this.registerSessionHook({
      type,
      fn,
      session: true,
  ...
  getService<T = unknown>(name: string) {
    ...
    return this.container.resolve<T>(name as keyof ReAuthCradleExtension);
  }
