    ...
    autoIntrospection: createAutoIntrospectionConfig({
      onRoutesGenerated: (routes: AutoGeneratedRoute[]) => {
        ...
        routes.forEach((route) => {
          console.log(`  ${route.method} ${route.path} -> ${route.pluginName}.${route.stepName}`);
...
export function createAdvancedContextAdapter() {
  const config: ExpressAdapterConfig = {
          ...
          switch (key) {
            ...
            case 'user-agent':
              ...
              return {
                ...
                browser: value.match(/(Chrome|Firefox|Safari|Edge)/)?.[1] || 'Unknown',
              };
    ...
    customRoutes: [
      ...
      createCustomRoute('GET', '/api/v1/auth/debug/context', (req: any, res: Response) => {
        res.json({
          cookies: req.cookies,
          headers: Object.fromEntries(
            Object.entries(req.headers).filter(([key]) => 
              key.startsWith('x-') || ['user-agent', 'referer', 'accept-language'].includes(key)
            )
          ),
      ...
      createContextRule('google-oauth', {
        ...
        transformOutput: (key, value, result, request) => {
          if (key === 'oauth_state' || key === 'oauth_code_verifier') {
            ...
            const tenantId = request.extractedContext?.tenant_id || request.params?.tenant;
            return {
              value,
    ...
    autoIntrospection: createAutoIntrospectionConfig({
      ...
      onRoutesGenerated: (routes) => {
        if (process.env.NODE_ENV === 'development') {
          console.log(`Generated ${routes.length} authentication routes with context rules`);
        }
      },
    }),
      ...
      createRouteOverride('google-oauth', 'callback', {
        handleResponse: (req: Request, res: Response, result: any) => {
          if (result.success) {
            ...
            const redirectUrl = process.env.FRONTEND_URL || 'https://app.example.com';
            return res.redirect(`${redirectUrl}/dashboard`);
          } else {
            const redirectUrl = process.env.FRONTEND_URL || 'https://app.example.com';
            return res.redirect(`${redirectUrl}/login?error=${encodeURIComponent(result.message)}`);
          }
        },
      }),
    ],
    ...
    contextRules: [
      ...
      createContextRule('email-password', {
        ...
        transformInput: (key, value, request) => {
          if (key === 'login_attempts') {
            const attempts = parseInt(value) || 0;
            console.log(`Login attempts for ${request.body?.email}: ${attempts}`);
            return attempts;
          }
