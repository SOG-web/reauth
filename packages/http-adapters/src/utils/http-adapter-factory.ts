import { ReAuthEngine, AuthOutput, Entity, AuthToken, AuthInput, AuthPlugin, AuthStep } from '@re-auth/reauth';
...
export interface HttpAdapterContext {
  engine: ReAuthEngine;
  config: Required<Omit<BaseHttpConfig, 'errorHandler'>> & { errorHandler?: (error: Error, context: any) => any };
 ...
 */
export function introspectReAuthEngine(engine: ReAuthEngine, config: AutoIntrospectionConfig = {}): AutoGeneratedRoute[] {
  ...
  plugins.forEach((plugin: AuthPlugin) => {
    ...
    plugin.steps.forEach((step: AuthStep<any>) => {
...
export function createHttpAdapter<T extends BaseHttpConfig>(
  ...
  return (engine: ReAuthEngine, config: T = {} as T) => {
    // Merge with defaults
    const mergedConfig: Required<Omit<BaseHttpConfig, 'errorHandler'>> & { errorHandler?: (error: Error, context: any) => any } = {
      basePath: '/auth',
  ...
  const introspectionHandler = async (request: any, response: any) => {
    try {
      ...
      if (response.json) {
        ...
        // Hono-like response - return Response object
...
function setupAutoGeneratedRoutes<T extends BaseHttpConfig>(
  ...
  context.autoGeneratedRoutes.forEach((autoRoute) => {
    // Check for route overrides
    const override = context.config.routeOverrides.find(
      (override) =>
        override.pluginName === autoRoute.pluginName && override.stepName === autoRoute.stepName,
    );
...
function setupCustomRoutes<T extends BaseHttpConfig>(
  ...
  context.config.customRoutes.forEach((route) => {
    const middleware = [
      ...(context.config.globalMiddleware || []),
      ...(route.middleware || []),
    ];
...
/**
 * Create a step handler for auto-generated routes with override support
 */
function createAutoGeneratedStepHandler<T extends BaseHttpConfig>(
  ...
  return async (request: any, response: any, next?: any): Promise<any> => {
    try {
      ...
      return context.errorResponse(response, error);
    }
  };
}
...
export const OAuth2ContextRules = {
   ...
   */
  pkce: (pluginName: string): ContextExtractionRule => createContextRule(pluginName, {
   ...
   */
  regular: (pluginName: string): ContextExtractionRule => createContextRule(pluginName, {
   ...
   */
  callback: (pluginName: string): ContextExtractionRule => createContextRule(pluginName, {
    stepName: 'callback',
    extractCookies: ['oauth_state', 'oauth_code_verifier'],
  }),
   ...
   */
  start: (pluginName: string): ContextExtractionRule => createContextRule(pluginName, {
    stepName: 'start',
    setCookies: ['oauth_state', 'oauth_code_verifier'],
  }),
}; 