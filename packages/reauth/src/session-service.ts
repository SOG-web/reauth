...
export class FumaSessionService implements SessionService {
  ...
  enableJWTFeatures(
    issuer: string = 'reauth',
    keyRotationIntervalDays: number = 10,
    keyGracePeriodDays: number = 2,
    defaultAccessTokenTtlSeconds: number = 15 * 60, // 15 minutes
    defaultRefreshTokenTtlSeconds: number = 30 * 24 * 60 * 60, // 30 days
    enableRefreshTokenRotation: boolean = true,
  ): void {
  ...
  async createJWTSession(
    ...
    const payload: ReAuthJWTPayload = {
      ...
      //TODO: add user data (this will come from the engine as an input by the dev)
    };

    return await this.jwtService.signJWT(payload);
  }

  async verifyJWTSession(token: string): Promise<{
    subject: any | null;
    token: string | null;
    ...
    try {
      ...
      const resolver = this.resolvers.get(payload.subject_type);
      if (!resolver) {
        return { subject: null, token: null, payload };
      }

      const version = await this.dbClient.version();
      const orm = this.dbClient.orm(version);

      const subject = await resolver.getById(payload.sub, orm);
      if (!subject) {
        return { subject: null, token, payload };
      }

      const safeSubject = resolver.sanitize
        ? resolver.sanitize(subject)
        : subject;
      return { subject: safeSubject, token, payload };
    } catch (error) {
  ...
  async createJWTTokenPair(
    ...
    return await this.jwtService.createTokenPair(payload, deviceInfo);
  }
  ...
  async revokeAllRefreshTokens(
    ...
    return await this.jwtService.revokeAllRefreshTokens(subjectType, subjectId, reason);
  }

  // Hybrid token verification - supports both JWT and legacy tokens
  async verifyAnyToken(token: string): Promise<{
    subject: any | null;
    token: string | null;
    type: 'jwt' | 'legacy';
    payload?: ReAuthJWTPayload;
    ...
    if (this.jwtService) {
      try {
        const result = await this.verifyJWTSession(token);
        if (result.subject) {
          return { ...result, type: 'jwt' };
        }
      } catch {
    ...
    const legacyResult = await this.verifySession(token);
    return {
      subject: legacyResult.subject,
      token: legacyResult.token,
      type: 'legacy',
    };
  }
