...
> = {
  ...
  protocol: 'generic-oauth.begin-oauth1-authorization.v1',
  ...
  async run(input, ctx) {
    ...
    try {
      ...
      const tokenSecretEncrypted = await TokenEncryption.encrypt(requestTokenResult.tokenSecret!);
      const expiresAt = new Date(Date.now() + 10 * 60 * 1000); // 10 minutes
      const tokenId = `oauth1_req_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

      try {
        await ctx.orm.insertOne('generic_oauth1_request_tokens', {
          id: tokenId,
