...
// Mock ORM for testing
const createMockOrm = () => ({
  count: vi.fn().mockResolvedValue(0),
  create: vi.fn().mockImplementation((table) => {
    if (table === 'subjects') {
      return Promise.resolve({ id: 'subject_123' });
    }
    return Promise.resolve({ id: 'session_123' });
  }),
...
const createMockEngine = (orm: any) => ({
  getOrm: vi.fn().mockResolvedValue(orm),
  createSessionFor: vi.fn().mockResolvedValue('mock_token_123'),
  checkSession: vi.fn().mockResolvedValue({
    valid: true,
    subject: { id: 'subject_123', type: 'guest' },
    token: 'mock_token_123',
  }),
});

// Mock context for testing
const createMockContext = (config?: Partial<AnonymousConfigV2>): StepContext<AnonymousConfigV2> => {
  const orm = createMockOrm();
  const engine = createMockEngine(orm);
...
describe('Anonymous Plugin V2 Step Integration', () => {
  let mockOrm: ReturnType<typeof createMockOrm>;
  let mockContext: StepContext<AnonymousConfigV2>;
