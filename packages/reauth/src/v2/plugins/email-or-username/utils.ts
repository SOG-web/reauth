...
export function findTestUser(
...
): { email?: string; username?: string; password: string; profile: Record<string, any> } | null {
  ...
  return config.testUsers.users.find(user => {
    if (user.password !== password) return false;
    
    if (inputType === 'email' && user.email === emailOrUsername) return true;
    if (inputType === 'username' && user.username === emailOrUsername) return true;
...
export function createEmailConfig(config: EmailOrUsernameConfigV2) {
  return {
    ...config.emailConfig,
    sessionTtlSeconds: config.sessionTtlSeconds ?? config.emailConfig?.sessionTtlSeconds,
    loginOnRegister: config.loginOnRegister ?? config.emailConfig?.loginOnRegister,
...
export function createUsernameConfig(config: EmailOrUsernameConfigV2) {
  return {
    ...
    sessionTtlSeconds: config.sessionTtlSeconds ?? config.usernameConfig?.sessionTtlSeconds,
    loginOnRegister: config.loginOnRegister ?? config.usernameConfig?.loginOnRegister,
    testUsers: config.testUsers?.enabled ? {
      enabled: true,
      users: config.testUsers.users
 ...
 */
export function transformToEmailInput(emailOrUsername: string, password: string, others?: Record<string, any>) {
  return {
 ...
 */
export function transformToUsernameInput(emailOrUsername: string, password: string, others?: Record<string, any>) {
  return {
 ...
 */
export function transformPluginOutput(output: any, inputType: 'email' | 'username', originalInput: string) {
  return {
    ...output,
