...
const emailOrUsernamePluginV2: AuthPluginV2<EmailOrUsernameConfigV2> = createAuthPluginV2<EmailOrUsernameConfigV2>(
  baseEmailOrUsernamePluginV2,
  {
    validateConfig: (config) => {
      const errs: string[] = [];
      
      // Validate email config if email verification is enabled
      if (config.emailConfig?.verifyEmail && typeof config.emailConfig?.sendCode !== 'function') {
        errs.push(
      ...
      if (config.detectionStrategy && !['auto', 'explicit'].includes(config.detectionStrategy)) {
        errs.push(
          "detectionStrategy must be 'auto' or 'explicit'",
        );
      }
      ...
      if (config.testUsers?.enabled && config.testUsers.users) {
        for (const user of config.testUsers.users) {
          ...
          if (!user.password) {
            errs.push(
              "Each test user must have a 'password' property",
            );
          }
        }
      }
