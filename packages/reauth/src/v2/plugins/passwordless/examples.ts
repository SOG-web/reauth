...
const magicLinksConfig: PasswordlessConfigV2 = {
  ...
  sendMagicLink: async (email: string, token: string, subject: any) => {
    ...
    console.log(`Magic link: https://example.com/auth/magic-link?token=${token}`);
...
const hybridConfig: PasswordlessConfigV2 = {
  ...
  sendMagicLink: async (email: string, token: string, subject: any) => {
    ...
    console.log(`📧 Magic link sent to ${email}: https://example.com/auth?token=${token}`);
  },
  webauthn: true,
  rpId: 'example.com',
  rpName: 'Example App',
...
export async function examplePasswordlessUsage() {
  ...
  console.log('2. Verify Magic Link: POST /auth/passwordless/verify-magic-link'); 
  console.log('3. Register WebAuthn: POST /auth/passwordless/register-webauthn');
  console.log('4. Authenticate WebAuthn: POST /auth/passwordless/authenticate-webauthn');
  console.log('5. List Credentials: GET /auth/passwordless/list-credentials');
  console.log('6. Revoke Credential: DELETE /auth/passwordless/revoke-credential');
...
export async function exampleStepExecution() {
  ...
  try {
    ...
    const registerResult = await engine.executeStep('passwordless', 'register-webauthn', {
      subject_id: 'user123',
