...
import type { ChangeMemberRoleInput, ChangeMemberRoleOutput, OrganizationConfigV2 } from '../types';
...
export const changeMemberRoleStep: AuthStepV2<
...
> = {
  ...
  inputs: ['token', 'organization_id', 'subject_id', 'role'],
  outputs: type({
    success: 'boolean',
    message: 'string',
    status: 'string',
  ...
  async run(input, ctx) {
    const { token, organization_id, subject_id, role } = input;
