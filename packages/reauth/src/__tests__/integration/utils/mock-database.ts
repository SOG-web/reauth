...
export class MockDatabase implements OrmLike {
  ...
  private createMockBuilder(records: any[]) {
    const conditions: Array<(record: any) => boolean> = [];

    const builder = (fieldOrMethod: string, ...args: any[]) => {
      // Handle method calls like 'and'
      if (fieldOrMethod === 'and') {
        // 'and' takes multiple condition arguments and combines them
        // For simplicity, we'll just add all conditions to the main list
        // In a real implementation, this would create nested logic
        return builder;
      }

      // Handle field conditions: b(field, operator, value)
      if (args.length === 2 && typeof args[0] === 'string') {
        const operator = args[0];
        const value = args[1];
        const field = fieldOrMethod;

        conditions.push((record: any) => {
    ...
    builder.getResults = () => {
      return records.filter(record => conditions.every(condition => condition(record)));
    };
