...
import sessionPlugin, { type SessionConfig, sessionSchema } from './plugin';

describe('Session Plugin  Integration', () => {
  ...
  it('should export session schema', () => {
    ...
    expect(sessionSchema.tables).toBeDefined();
    expect(sessionSchema.tables?.session_devices).toBeDefined();
    expect(sessionSchema.tables?.session_metadata).toBeDefined();
  });
  ...
  it('should validate configuration correctly', () => {
    ...
    expect(typeof sessionPlugin.config?.maxConcurrentSessions).toBe('number');
    expect(typeof sessionPlugin.config?.deviceTrackingEnabled).toBe('boolean');
    expect(typeof sessionPlugin.config?.cleanupIntervalMinutes).toBe('number');
    expect(typeof sessionPlugin.config?.sessionRetentionDays).toBe('number');
  });
