 ...
 */
export const calculateExpiresAt = (config?: AnonymousConfig): Date => {
...
export const cleanupExpiredSessions = async (
  ...
  const subjectRetentionDays =
    config?.guestSubjectRetentionDays ?? sessionRetentionDays;

  const sessionCutoffDate = new Date(
    now.getTime() - sessionRetentionDays * 24 * 60 * 60 * 1000,
  );
  const subjectCutoffDate = new Date(
  ...
  const sessionResult = await orm.deleteMany('anonymous_sessions', {
    where: (b: any) =>
      b.and(b('expires_at', '<', now), b('created_at', '<', sessionCutoffDate)),
  });
