...
export function findTestUser(
  ...
  if (!config.testUsers.checkEnvironment(config.testUsers.environment))
    return null;
 ...
 */
export function createEmailConfig(config: EmailOrUsernameConfig) {
  return {
    ...(config.emailConfig ?? {}),
    sessionTtlSeconds:
 ...
 */
export function createUsernameConfig(config: EmailOrUsernameConfig) {
  return {
