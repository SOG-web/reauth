 ...
 */
export const calculateInvitationExpiry = (
...
export const getOrganizationChildren = async (
  ...
  return (
    (await orm.findMany('organizations', {
      where: (b: any) =>
        b.and(b('parent_id', '=', organizationId), b('is_active', '=', true)),
    })) || []
  );
};
...
export const cleanupExpiredInvitations = async (
  ...
  try {
    ...
    const expiredResult = await (orm as any).deleteMany(
      'organization_invitations',
      {
        where: (b: any) =>
          b.and(
            b.or(
              b.and(b('status', '=', 'expired')),
              b.and(b('status', '=', 'pending'), b('expires_at', '<', now)),
            ),
          ),
      },
    );
    ...
    expiredInvitationsDeleted =
      typeof expiredResult === 'number' ? expiredResult : 0;

    // Step 2: Delete old revoked invitations
    const revokedResult = await (orm as any).deleteMany(
      'organization_invitations',
      {
        where: (b: any) => b.and(b('status', '=', 'revoked')),
      },
    );

    revokedInvitationsDeleted =
      typeof revokedResult === 'number' ? revokedResult : 0;
