...
export async function cleanupExpiredAuditLogs(orm: OrmLike, config?: AdminConfig) {
  const retentionDays = config?.auditLogRetentionDays || 90;
  const cutoffDate = new Date();
  cutoffDate.setDate(cutoffDate.getDate() - retentionDays);

  const result = await orm.delete('audit_logs', {
...
export async function getAdminPermissions(orm: OrmLike, subjectId: string, config?: AdminConfig) {
