...
import passwordlessPlugin, { type PasswordlessConfig } from './plugin';
import { ReAuthEngine } from '../../engine';

// Example 1: Magic Links Only
const magicLinksConfig: PasswordlessConfig = {
  magicLinks: true,
...
const webAuthnConfig: PasswordlessConfig = {
  webauthn: true,
  rpId: 'example.com',
  rpName: 'Example App',
...
const hybridConfig: PasswordlessConfig = {
  magicLinks: true,
  sendMagicLink: async (email: string, token: string, subject: any) => {
...
export async function examplePasswordlessUsage() {
  ...
  const engine = new ReAuthEngine({
    ...
    plugins: [
      passwordlessPlugin, // Type assertion to bypass complex generic constraints
    ],
  });
