...
import { createReAuthHttpAdapter } from '../src/utils/factory.js';
...
const createMockEngineV2 = (): ReAuthEngineV2 => ({
  ...
  executeStep: async function(pluginName: string, stepName: string, input: any) {
  ...
  createSessionFor: async (subjectType: string, subjectId: string, ttlSeconds?: number) => {
...
describe('V2 HTTP Adapter Integration Tests', () => {
  ...
  describe('Adapter Initialization', () => {
    ...
    it('should discover V2 plugin endpoints', () => {
      ...
      const emailPasswordEndpoints = endpoints.filter(e => e.pluginName === 'email-password');
      expect(emailPasswordEndpoints).toHaveLength(1);
      expect(emailPasswordEndpoints[0].stepName).toBe('login');
      
      const apiKeyEndpoints = endpoints.filter(e => e.pluginName === 'api-key');
  ...
  describe('Authentication Step Execution', () => {
    ...
    it('should handle non-existent plugin/step', async () => {
      ...
      await expect(adapter.executeAuthStep(request as any))
        .rejects.toThrow('Endpoint not found: non-existent/invalid');
    });
  });
});