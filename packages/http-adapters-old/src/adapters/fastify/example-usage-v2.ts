...
/**
 * Basic Fastify adapter usage with  factory pattern
 */
...
export async function createOAuthFastifyAdapter(
  ...
  const createAdapter = createFastifyAdapter(fastify, frameworkAdapter);
...
export async function createReAuthFastifyPlugin(reAuthEngine: ReAuthEngine) {
  return FastifyAdapter.createPlugin(reAuthEngine, {
    basePath: '/auth',
