...
export default function TestClientPage() {
    ...
    const handleLogin = async () => {
        const { data, error } = await authClient.emailAuthPlugin.login({
            ...
            onSuccess: (data: any) => {
                if (data.entity) {
                    setMessage(data.transformedMessage || `Login successful! Welcome, ${data.entity.email}.`);
                }
            },
            ...
            interceptors: {
                response: (response: any) => {
                    ...
                    if (response.data.data.success) {
                        response.data.data.transformedMessage = 'Login successful (transformed by interceptor)!';
                    }
                    return response;
                }
            }
    ...
    const handleRegister = async () => {
        await authClient.emailAuthPlugin.register({
            ...
            onRequest: () => setMessage('Loading...'),
            onSuccess: (data) => setMessage(`Registration successful! Please check your email to verify.`),
