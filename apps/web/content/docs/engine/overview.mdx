...
flowchart TB
    subgraph Pattern["ReAuth Universal Pattern"]
        direction TB
        Logic["🧠 Business Logic<br/>(Transport Agnostic)"]
        Protocol["🔌 Protocol Layer<br/>(HTTP, GraphQL, gRPC, etc.)"]
        Storage["💾 Storage Layer<br/>(SQL, NoSQL, KV, etc.)"]
  ...
  authHooks: [
    // Engine-level hooks (run for all flows)
    {
      ...
      fn: async (input, container) => {
        ...
        logger.info('engine', 'Before any auth flow', { input });
        return input;
      },
    },
  ],
  sessionHooks: [
    // Session-level hooks
    {
      ...
      fn: async (data, container) => {
        const logger = container.resolve('logger');
        logger.info('session', 'After session created', { data });
        return data;
      },
    },
  ],
...
- `logger`: Logger instance for structured logging (required)
  ...
  authHooks: [
    {
      ...
      fn: async (input, container) => {
        ...
        const logger = container.resolve('logger');
        logger.info('engine', 'Before any auth step', { input });
        return input;
      },
    },
  ],
...
const myPlugin = {
  ...
  rootHooks: {
    before: async (input, ctx, step) => {
      ...
      logger.info('plugin', `Executing step: ${step.name}`, { input });
      return input;
    },
    after: async (output, ctx, step) => {
      // Post-process output
      const logger = ctx.engine.getContainer().resolve('logger');
      logger.info('plugin', `Completed step: ${step.name}`, { output });
      return output;
    },
    onError: async (error, input, ctx, step) => {
      // Handle errors
      const logger = ctx.engine.getContainer().resolve('logger');
      logger.error('plugin', `Error in ${step.name}`, { error, input });
    },
  },
  steps: [/* ... */],
};
...
const registerStep = {
  ...
  hooks: {
    before: async (input, ctx) => {
      ...
      const logger = ctx.engine.getContainer().resolve('logger');
      logger.info('step', 'Before registration', { input });
    },
    after: async (output, ctx) => {
      // Step-specific post-processing  
      const logger = ctx.engine.getContainer().resolve('logger');
      logger.info('step', 'After registration', { output });
    },
    onError: async (error, ctx) => {
      // Step-specific error handling
      const logger = ctx.engine.getContainer().resolve('logger');
      logger.error('step', 'Registration failed', { error });
    },
  },
  async run(input, ctx) {
    // Step implementation
  ...
  authHooks: [
    {
      ...
      fn: async (input, container) => {
        ...
        const logger = container.resolve('logger');
        logger.info('engine', 'Authentication attempt', {
...
const myPlugin = {
  ...
  rootHooks: {
    onError: async (error, input, ctx, step) => {
      ...
      const logger = ctx.engine.getContainer().resolve('logger');
      logger.error('plugin', 'Plugin error', {
        plugin: 'my-plugin',
        step: step.name,
...
engine.registerCleanupTask({
  ...
  async runner(orm, config) {
    ...
    logger.info('cleanup', 'Cleanup task completed', {
      task: 'expired-tokens',
