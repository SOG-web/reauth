...
| `logger`                 | `LoggerInterface` | Yes      | Logger instance for structured logging                  |
...
ReAuth requires a logger instance for structured logging. Create a logger using the `@re-auth/logger` package:
...
- `prefix`: Application identifier for log messages
- `prefixEnv`: Environment variable prefix for tag control (e.g., `REAUTH_DEBUG`)
- `enabledTags`: Array of log tags to enable by default
- `timestampFormat`: `'iso'` for ISO timestamps or `'human'` for readable format
- `emojis`: Enable/disable emoji icons in log messages
- `fileOutput`: Optional file logging configuration
  ...
  authHooks: [
    {
      ...
      fn: async (input, container) => {
        ...
        logger.info('engine', 'Before any authentication step', { input });
        return input;
      },
    },
    {
      ...
      fn: async (output, container) => {
        const logger = container.resolve('logger');
        logger.info('engine', 'After any authentication step', { output });
        return output;
      },
    },
  ],
  ...
  sessionHooks: [
    {
      ...
      fn: async (data, container) => {
        const logger = container.resolve('logger');
        logger.info('session', 'Before session creation/verification', {
          data,
        });
        return data;
      },
    },
    {
      ...
      fn: async (data, container) => {
        const logger = container.resolve('logger');
        logger.info('session', 'After session creation/verification', { data });
        return data;
      },
    },
  ],
...
- [ ] Logger instance created and passed to ReAuth engine
...
- [ ] Environment variables for logger tag control (e.g., `REAUTH_DEBUG`) configured
...
// Create logger instance
const logger = createDefaultLogger({
  prefix: 'ProductionApp',
  prefixEnv: 'REAUTH_',
  enabledTags: ['auth', 'session', 'plugin', 'database'],
  timestampFormat: 'iso',
  emojis: false, // Professional output for production
});
  ...
  authHooks: [
    {
      ...
      fn: async (input, container) => {
        ...
        const logger = container.resolve('logger');
        logger.info('engine', 'Authentication attempt', { input });
        return input;
      },
    },
  ],
  ...
  sessionHooks: [
    {
      ...
      fn: async (data, container) => {
        ...
        const logger = container.resolve('logger');
        logger.info('session', 'Session created', { userId: data.subject?.id });
