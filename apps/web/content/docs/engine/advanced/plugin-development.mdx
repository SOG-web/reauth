...
Every plugin implements the `AuthPlugin<Cfg>` interface. ReAuth uses a base plugin + factory pattern:
...
// 1. Define the base plugin structure
export const baseMyAuthPlugin = {
  name: 'my-auth-plugin' as const,
  ...
  initialize(engine) {
    ...
    const cleanupIntervalMs = (config.cleanupIntervalMinutes || 60) * 60 * 1000;
...
} satisfies AuthPlugin<MyPluginConfig, 'my-auth-plugin'>;

// 2. Create factory function that merges config and allows step overrides
export const myAuthPlugin = (
  ...
  options?: {
    ...
    rootHooks?: {
      before?: (input: any, ctx: any, step: any) => Promise<any> | any;
      after?: (output: any, ctx: any, step: any) => Promise<any> | any;
      onError?: (
        error: unknown,
        input: any,
        ctx: any,
  ...
  const pl = createAuthPlugin(baseMyAuthPlugin, {
    config,
    stepOverrides: options?.stepOverrides,
    validateConfig: (config) => {
      const errs: string[] = [];

      if (config.requireVerification && !config.sendCode) {
        errs.push(
          'requireVerification is true but sendCode function is not provided',
        );
      }
...
> = {
  ...
  async run(input, ctx) {
    ...
    // Create subject and credentials
    ...
    if (ctx.config.requireVerification && ctx.config.sendCode) {
    ...
    // Create session if login on register is enabled
    ...
    const outSubject = {
      ...
      email,
      provider: 'email',
      verified: !ctx.config.requireVerification,
    };
  ...
  hooks: {
    ...
    after: async (output, ctx) => {
      ...
      if (output.success) {
        console.log(`User registered successfully: ${output.subject?.id}`);
      }
    },
...
export const registerStep: AuthStep<MyPluginConfig> = {
  ...
  async run(input, ctx: StepContext<MyPluginConfig>) {
    ...
    // 4. Create email identity record
    await orm.create('email_identities', {
      identity_id: identity.id,
    });

    // 5. Send verification code if configured
    if (ctx.config.sendCode) {
      const code = generateCode();
      await ctx.config.sendCode({ id: subject.id }, code, input.email, 'verify');
    }

    // 6. Create session if login on register is enabled
    const token = loginOnRegister
      ? await ctx.engine.createSessionFor('subject', subject.id as string, ttl)
      : undefined;
...
- `ctx.engine`: The ReAuth engine instance with methods like `getOrm()`, `createSessionFor()`, etc.
- `ctx.config`: The merged plugin configuration (defaults + user config)
...
import { myAuthPlugin, myPluginSchema } from './my-plugin';
...
describe('MyAuthPlugin', () => {
  ...
  beforeEach(async () => {
    // Setup test database with plugin schema
    const { schema: v1 } = reauthDb('1.0.1', [myPluginSchema]);
    const factory = reauthDbVersions([v1]);
    const client = factory.client(
      kyselyAdapter({ provider: 'sqlite', db: ':memory:' }),
    );
...
export const baseSmsAuthPlugin = {
  name: 'sms-auth' as const,
  ...
  initialize(engine) {
    ...
    const config = this.config || {};
    const cleanupIntervalMs = (config.cleanupIntervalMinutes || 5) * 60 * 1000;
...
} satisfies AuthPlugin<SMSAuthConfig, 'sms-auth'>;
...
export const smsAuthPlugin = (
  ...
  const pl = createAuthPlugin(baseSmsAuthPlugin, {
    config,
    validateConfig: (config) => {
      const errs: string[] = [];

      if (!config.apiKey) {
        errs.push('apiKey is required for SMS authentication');
      }

      if (!config.from) {
        errs.push('from phone number is required for SMS authentication');
      }
...
import { kyselyAdapter } from 'fumadb/adapters/kysely';
import { smsAuthPlugin, smsAuthSchema } from './sms-auth-plugin';

// Setup database with plugin schema
const { schema: v1 } = reauthDb('1.0.1', [smsAuthSchema]);
const factory = reauthDbVersions([v1]);
const client = factory.client(
